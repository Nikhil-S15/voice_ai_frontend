...
class LarynxHypopharynxPage extends StatefulWidget {
  final String userId;
  final String sessionId;
  const LarynxHypopharynxPage({
    Key? key,
    required this.userId,
    required this.sessionId,
  }) : super(key: key);
  @override
  State<LarynxHypopharynxPage> createState() => _LarynxHypopharynxPageState();
}

class _LarynxHypopharynxPageState extends State<LarynxHypopharynxPage> {
  ...
  Future<void> _restoreProgress() async {
    ...
    if (saved != null) {
      ...
      setState(() {
        ...
        mStageController.text = data['histogram']?['mStage'] ?? '';
        ...
        reconstructionType = data['treatmentDetails']?['reconstruction'];
          ...
          children: [
            ...
            if (diagnosis == 'Yes') ...[
              AppCheckboxGroup(
                ...
                options: const ['Laryngoscopy', 'Biopsy/pathology', 'Imaging'],
            ...
            if (diagnosis == 'Yes') ...[
              ...
              if (treatmentModalities.contains('Radiotherapy')) ...[
                ...
                AppCheckboxGroup(
                  ...
                  options: const ['Primary', 'Neck', 'Both'],
            ...
            // Follow-up Section
            SectionHeader(title: 'Follow-up', icon: Icons.calendar_today),
            AppTextFormField(
            ...
            if (diagnosis == 'Yes') ...[
              ...
              AppRadioGroup(
                label: 'Speech status:',
                options: const ['Normal', 'Impaired', 'Non-verbal'],
                value: speechStatus,
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      // 1. SAVE partial progress for backend resume
      final progressData = Map<String, dynamic>.from(
        cleanPayload,
      ); // You may remove keys you don't want to save as draft if desired
      await ApiService.saveSessionProgress(
        userId: widget.userId,
        sessionId: widget.sessionId,
        currentPage: '/pharynx-cancer', // Next step route
        progressData: progressData,
      );

      // 2. Submit form as before
      debugPrint('Final payload: ${jsonEncode(cleanPayload)}');
      final response = await ApiService.submitLarynxHypopharynxData(
        cleanPayload,
      );
